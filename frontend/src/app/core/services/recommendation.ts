...
export class RecommendationService {
  ...
  getRecommendations(consultantId: number): Observable<any[]> {
    return this.http.get<any>(`http://localhost:5000/api/consultants/${consultantId}`).pipe(
      switchMap((consultant) => {
        const cv = consultant.cv;
        const payload = { query: cv };
        return this.http.post<any>('http://localhost:5000/recommend', payload);
      }),
      switchMap((response: any) => {
        ...
        const jobRequests = jobIds.map((id: number) =>
          this.http.get<any>(`http://localhost:5000/api/jobs/${id}`)
        );
  ...
  getConsultants(): Observable<any[]> {
    return this.http.get<any[]>(`http://localhost:5000/api/consultants`);
  }
}
